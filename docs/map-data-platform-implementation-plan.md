# 땅뷰 지도·부동산 데이터 플랫폼 구현 계획

- 조사 기준일: 2026-07-22
- 대상: 현재 Next.js + Vercel 기반 땅뷰 웹사이트와 이후 모바일 앱
...
땅뷰의 차별점은 지도 위에 마커를 많이 올리는 것이 아니라, 한 필지를 선택했을 때 **거래·규제·계획·공매·경매를 출처와 신뢰도까지 포함한 ‘필지 리포트’로 묶어 보여주는 것**이어야 한다.
...
- 사용자는 땅뷰에서 주변 실거래·토지이용·개발계획을 보고, 상세 법적 문서는 법원경매 공식 원문에서 확인하게 한다.
