#!/usr/bin/env python3
"""Add representative VWorld address coordinates to normalized Incheon projects."""
...
def search_queries(project: dict[str, object]) -> list[str]:
    ...
    return list(dict.fromkeys(query for query in (base, simplified, first_lot) if query))


def geocode(api_key: str, project: dict[str, object]) -> dict[str, object] | None:
        ...
        for category in ("parcel", "road"):
            ...
            request = urllib.request.Request(
                ...
                headers={
                    ...
                    "User-Agent": "LandView data updater",
                },
            )
            ...
            for item in items:
                ...
                if "인천광역시" not in parcel_address:
                    continue
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=max(1, min(args.workers, 10))) as executor:
        futures = {executor.submit(geocode, api_key, project): project for project in pending}
